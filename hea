...
# Data: [poloměr r (Å), teplota tání Tm (K), elektronegativita chi]
ELEMENT_DATA = {
...
# Odhadované entalpie míšení dH_mix (kJ/mol) - Miedema model values
MIXING_ENTHALPY = {
...
def calculate_hea_properties(composition):
    ...
    # Normalizace (pro jistotu, aby suma byla 1)
    x = x / np.sum(x)
...
def run_sensitivity_analysis(target_element='Si'):
    ...
    concentrations = np.linspace(0, 0.25, 50)  # od 0 do 25 % cílového prvku
    results = {'delta': [], 'omega': [], 'dh_mix': []}

    for c in concentrations:
        # Rozděl zbytek (1-c) rovnoměrně mezi ostatní 4 prvky
